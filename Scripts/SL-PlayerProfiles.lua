...
FindProfileByHash = function(hash)
	for _, id in ipairs(PROFILEMAN:GetLocalProfileIDs()) do
		local profile = PROFILEMAN:GetLocalProfile(id)
		local dir = PROFILEMAN:LocalProfileIDToDir(id)
		-- We require an explicit profile to be loaded.
		if not dir or #dir == 0 then return "" end
	
		local path = dir.. "CardSwipe.ini"
		...
		if not FILEMAN:DoesFileExist(path) then
			...
			for k,v in pairs(contents["Card Swipe"]) do
				if k == "ID" then
					--SM(v .. "\n".. hash)
					if v == hash then
						return PROFILEMAN:GetLocalProfileIndexFromID(id)
					else
						--SM("Profile Not Found!")
					end
				end
			end
		end
		...
		for k,v in pairs(contents["Card Swipe"]) do
			if k == "ID" then
				if #v ~= 256 then
					...
					if #v ~= 0 then
						SM(ToEnumShortString(player).." has invalid Hash length!")
					end
					SL[pn].ID= ""
				else
					SL[pn].ID = v
