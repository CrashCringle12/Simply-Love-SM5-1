...
local t = Def.ActorFrame{
	OnCommand=function(self)
		if IsUsingWideScreen() then
			self:zoom(0.7655)
			self:xy(_screen.cx - 170, ThemePrefs.Get("SelectMusicDisplayStyle") == "ITG+" and 96 or 112)
		else
			self:zoom(0.72)
			self:xy(_screen.cx - 160, ThemePrefs.Get("SelectMusicDisplayStyle") == "ITG+" and 96 or 112)
		end
	end
}
